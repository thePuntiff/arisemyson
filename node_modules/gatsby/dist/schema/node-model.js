...
var _runFastFilters = require("../redux/run-fast-filters");
...
class LocalNodeModel {
  ...
  createPageDependency(createPageDependencyArgs) {
    if (createPageDependencyArgs.connection) {
      const nodeTypeNames = toNodeTypeNames(this.schema, createPageDependencyArgs.connection);
  ...
  getAllNodes(args, pageDependencies) {
    ...
    if (!type) {
      result = (0, _nodes.getNodes)();
    } else {
      const nodeTypeNames = toNodeTypeNames(this.schema, type);
      const nodes = nodeTypeNames.reduce((acc, typeName) => {
        acc.push(...(0, _nodes.getNodesByType)(typeName));
        return acc;
      }, []);
  ...
  async runQuery(args, pageDependencies) {
    ...
    const queryResult = await (0, _runFastFilters.runFastFiltersAndSort)({
      queryArgs: query,
      firstOnly,
    ...
    if (tracer) {
      trackInlineObjectsActivity = reporter.phantomActivity(`trackInlineObjects`, {
        parentSpan: tracer.getParentActivity().span
      });
    ...
    if (firstOnly) {
      ...
      if (((_result = result) === null || _result === void 0 ? void 0 : _result.length) > 0) {
        ...
        result = null; // Couldn't find matching node.
  ...
  async _doResolvePrepareNodesQueue(type, nodeTypeNames) {
    ...
    if (!_.isEmpty(actualFieldsToResolve)) {
      await (0, _nodes.saveResolvedNodes)(nodeTypeNames, async node => {
        this.trackInlineObjectsInRootNode(node);
...
class ContextualNodeModel {
  ...
  withContext(context) {
    return new ContextualNodeModel(this.nodeModel, { ...this.context,
      ...context
    });
  }
...
const getNodeById = id => id != null ? (0, _nodes.getNode)(id) : null;
...
async function resolveRecursive(nodeModel, schemaComposer, schema, node, type, queryFields, fieldsToResolve) {
  ...
  for (const fieldName of Object.keys(fieldsToResolve)) {
    ...
    let innerValue = await resolveField(nodeModel, schemaComposer, schema, node, gqlField, fieldName);
    ...
    if (gqlField && innerValue != null) {
      if (isCompositeType(gqlFieldType) && !(gqlNonNullType instanceof GraphQLList)) {
        ...
        innerValue = await Promise.all(innerValue.map(item => item == null ? item : resolveRecursive(nodeModel, schemaComposer, schema, item, gqlFieldType, queryField, _.isObject(fieldToResolve) ? fieldToResolve : queryField)));
      }
    }
  ...
  for (const fieldName of Object.keys(queryFields)) {
    if (!fieldsToResolve[fieldName] && node[fieldName]) {
      ...
      resolvedFields[fieldName] = await resolveField(nodeModel, schemaComposer, schema, node, gqlFields[fieldName], fieldName);
    }
  }
...
function resolveField(nodeModel, schemaComposer, schema, node, gqlField, fieldName) {
  if (!(gqlField === null || gqlField === void 0 ? void 0 : gqlField.resolve)) {
...
const determineResolvableFields = (schemaComposer, schema, type, fields, nodeTypeNames, isNestedType = false) => {
  ...
  Object.keys(fields).forEach(fieldName => {
    ...
    if (_.isObject(field) && gqlField) {
      const innerResolved = determineResolvableFields(schemaComposer, schema, gqlFieldType, field, toNodeTypeNames(schema, gqlFieldType), true);
...
const addRootNodeToInlineObject = (rootNodeMap, data, nodeId, isNode
/* : boolean */
, path
/* : Set<mixed> */
) =>
